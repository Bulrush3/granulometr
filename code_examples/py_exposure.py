...
def configure_exposure_acquire_images(device, nodes, initial_vals):
	...
	'''
	Получить узел времени экспозиции
	Чтобы получить максимальное и минимальное значения времени экспозиции, получите
	узел времени экспозиции. Неудачные попытки получить узел возвращают значение null, поэтому проверьте
	, что узел существует. И поскольку мы ожидаем установить его значение, убедитесь, что
	узел времени экспозиции доступен для записи.
	'''
	...
	'''
	Установите время экспозиции
	Перед установкой времени экспозиции убедитесь, что новое время экспозиции не
	выходит за пределы допустимого диапазона времени экспозиции. Если значение выше максимального или
	ниже минимального, обновите значение, чтобы оно было в пределах диапазона. Наконец, установите новый
	время экспозиции.
	'''
	print(nodes['ExposureTime'].min)
	print(nodes['ExposureTime'].max)
